...
import { classifyStudent } from './students.js'; 
import { 
...
let currentLogs = {}; // Cache para sa mabilis na Export
    ...
    container.innerHTML = `
        <div class="module-header" style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; padding:0 10px;">
            <div>
                ...
                <p style="margin:0; font-size:12px; color:#64748b;">Multiple Officers Mode Active</p>
            </div>
            <div style="display:flex; gap:10px;">
                <select id="attendance-event-id" class="swal2-input" style="margin:0; width:200px; height:45px; font-size:13px; border-radius:10px;">
                    <option value="">-- Select Event --</option>
                    ${eventOptions}
                </select>
                <button id="btn-toggle-camera" class="btn-gold" style="height:45px; width:150px; border-radius:10px; font-weight:700;">
        ...
        <div class="dashboard-grid" style="grid-template-columns: 1fr 1.8fr; gap:15px; padding:0 10px;">
            <div style="display:flex; flex-direction:column; gap:15px;">
                <div class="dashboard-card" style="padding:10px; background:#0f172a; border-radius:20px; min-height:280px; position:relative; overflow:hidden; border:4px solid #1e293b;">
                    ...
                    <div id="camera-placeholder" style="width:100%; height:280px; display:flex; flex-direction:column; align-items:center; justify-content:center; color:#475569;">
                        <i data-lucide="aperture" class="spin" style="width:50px; height:50px; mb:10px; opacity:0.3;"></i>
                ...
                <div id="scan-feedback" class="dashboard-card" style="text-align:center; transition: 0.3s ease; border:2px solid #e2e8f0; min-height:120px; display:flex; flex-direction:column; justify-content:center;">
                    ...
                    <p id="scan-status" style="margin:5px 0 0 0; font-weight:700; font-size:0.9rem; color:#64748b; text-transform:uppercase;">Waiting for ID...</p>
                </div>
            ...
            <div class="dashboard-card" style="padding:15px;">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
                    <h3 style="margin:0; font-size:15px; display:flex; align-items:center; gap:8px;"><i data-lucide="clipboard-check"></i> Live List</h3>
                    <div style="display:flex; gap:10px; align-items:center;">
                        <button id="btn-export-csv" class="btn-gold" style="padding:0 15px; height:32px; font-size:11px; border-radius:8px;">
...
function setupEventListeners() {
    ...
    document.getElementById('btn-export-csv').onclick = exportToCSV; // Re-linked
...
/**
 * UPDATED CSV EXPORT (SPEED OPTIMIZED)
 */
async function exportToCSV() {
    ...
    // Mas mabilis na pagkuha ng lahat ng logs para sa event na ito
    const snap = await getDocs(query(collection(db, "attendance"), where("eventId", "==", eventId)));
    
    if (snap.empty) return Swal.fire("Empty", "No attendance records yet.", "info");

    let csv = "ID Number,Student Name,Department,Time In,Time Out,Status\n";
    snap.forEach(d => {
        const v = d.data();
    ...
    link.download = `Attendance_${event.name.replace(/\s+/g, '_')}.csv`;
    link.click();
}
...
async function refreshAttendanceTable() {
    ...
    tbody.innerHTML = `<tr><td colspan="4" style="text-align:center; padding:20px;">Updating List...</td></tr>`;
...
async function handleAttendanceInput(studentId) {
    ...
    try {
        ...
        if (!attendSnap.exists()) {
            ...
            updateUI(student.fullName, "DONE LOGGING", "warning");
        }
        beep.play();
    } catch (err) { console.error(err); }
}
...
async function toggleCameraScanner(btn) {
    ...
    if (isScannerActive) {
        ...
        try {
            ...
            await html5QrCode.start({ facingMode: "environment" }, { fps: 20, qrbox: 220 }, (text) => {
        ...
        } catch (err) { console.error(err); span.innerText = "Open Camera"; }
    }
}
...
function updateUI(name, status, type) {
    ...
    setTimeout(() => { idEl.innerText = "READY"; stEl.innerText = "Waiting for ID..."; box.style.background = "white"; }, 3000);
}
